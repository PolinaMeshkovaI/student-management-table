...
function createStudentTr(student) {
  ...
  $studentThAge.textContent = `${formatDate(student.birthday)} (${calculateAge(
    student.birthday
  )})`;
...
$form.addEventListener("submit", function (event) {
  ...
  if ($inputFaculty.value.trim() == "") {
    $alert.classList.remove("alert-invisible");
    $alert.textContent = "Факультет не введён. Введите факультет, пожалуйста.";
    return;
  ...
  savedStubentsJSON = localStorage.getItem("studentData");
  savedStudentData = JSON.parse(savedStubentsJSON);
